 ...
 * `SAST (Static Application Security Testing) – White-box testing` - Analyzes source code, scripts, or IaC (Infrastructure as Code) without running the program. It detects security flaws, misconfigurations, or   bad coding patterns early in the development lifecycle. White-box testing means testing with knowledge of internal code and structure (like reading `.tf` or `.py` files). <br/>
