...
[In modern DevSecOps/DevOps, automating infrastructure and application delivery](https://www.redhat.com/en/topics/devops/what-is-devsecops) demands integrated quality and security checks within CI/CD pipelines. Tools like TFLint, tfsec, and SonarCloud enable automated linting, static analysis, and vulnerability scanning for Terraform and application code. <br/>
  ...
  * This blog demonstrates how to implement `TFLint` and `tfsec` in a GitHub Actions pipeline and demostrates how the setup can be extended to `SonarCloud` & `Database DevOps` for broader application analysis & streamlining delivery. <br/>
# Application Security Testing in DevSecOps (SAST vs DAST)
In modern DevSecOps pipelines, [application security testing](https://circleci.com/blog/sast-vs-dast-when-to-use-them/) is essential to detect vulnerabilities early and continuously. [Two key techniques are SAST and DAST](https://about.gitlab.com/topics/devsecops/sast-vs-dast/): <br/>
 * `SAST (Static Application Security Testing) – White-box testing` - Analyzes source code,configuration files, scripts, or IaC (Infrastructure as Code) without running the program to detect security vulnerabilities, misconfigurations, bugs, and anti-patterns early in the development lifecycle. SAST tools are integrated into CI/CD pipelines to automatically scan for issues every time code is committed — helping shift security left and catch problems before they reach production. White-box testing means testing with knowledge of internal code and structure (like reading `.tf` or `.py` files). <br/>
